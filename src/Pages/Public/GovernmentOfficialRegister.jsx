...
import {
    ...
    Upload, ArrowLeft, AlertCircle, CheckCircle, FileText, Briefcase
} from "lucide-react";
...
const GovernmentOfficialRegister = () => {
    ...
    // Form state
    const [officialType, setOfficialType] = useState("");  // "MLA" or "DEPARTMENT_OFFICIAL"
    const [name, setName] = useState("");
    const [email, setEmail] = useState("");
    const [phone, setPhone] = useState("");
    const [state, setState] = useState("");
    const [district, setDistrict] = useState("");  // For department officials
    const [constituency, setConstituency] = useState("");  // For MLAs
    const [department, setDepartment] = useState("");  // For department officials
    const [designation, setDesignation] = useState("");  // For department officials
    const [partyName, setPartyName] = useState("");  // For MLAs
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // 4. Create application based on official type
            if (officialType === "MLA") {
                // Create MLA application
                ...
                // Create Department Official application
                // Note: You may want to create a separate "official_applications" collection
                // For now, storing in the same MLA applications with a type field
                await databases.createDocument(
                    import.meta.env.VITE_DATABASE_ID,
                    import.meta.env.VITE_MLA_APPLICATIONS_COLLECTION_ID,
                    ID.unique(),
                    {
                        ...
                        partyName: "", // Not applicable for officials
                        govtIdProof: fileUrl.href,
                        verificationStatus: "pending",
                        appliedAt: new Date().toISOString(),
                    }
            ...
            <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-pink-50 flex items-center justify-center p-6">
                <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
                    ...
                    <p className="text-gray-600 mb-4">
                        Your {officialType === "MLA" ? "MLA" : "Department Official"} application has been submitted successfully. Our admin team will review your application and notify you via email.
                    </p>
            ...
            <div className="relative w-full max-w-2xl">
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-8 border border-gray-100">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">Government Official Registration</h1>
                        <p className="text-gray-600">Apply for portal access as MLA or Department Official</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <button
                                    type="button"
                                    onClick={() => setOfficialType("MLA")}
                                    className={`p-4 border-2 rounded-lg transition-all text-left ${officialType === "MLA"
                                            ? "border-purple-500 bg-purple-50"
                                            : "border-gray-200 hover:border-purple-300"
                                        }`}
                                >
                                    <Shield className={`w-6 h-6 mb-2 ${officialType === "MLA" ? "text-purple-600" : "text-gray-400"}`} />
                                    <div className="font-semibold text-gray-900">MLA</div>
                                    <div className="text-xs text-gray-600">Member of Legislative Assembly</div>
                                </button>

                                <button
                                    type="button"
                                    onClick={() => setOfficialType("DEPARTMENT_OFFICIAL")}
                                    className={`p-4 border-2 rounded-lg transition-all text-left ${officialType === "DEPARTMENT_OFFICIAL"
                                            ? "border-purple-500 bg-purple-50"
                                            : "border-gray-200 hover:border-purple-300"
                                        }`}
                                >
                                    <Briefcase className={`w-6 h-6 mb-2 ${officialType === "DEPARTMENT_OFFICIAL" ? "text-purple-600" : "text-gray-400"}`} />
                                    <div className="font-semibold text-gray-900">Department Official</div>
                                    <div className="text-xs text-gray-600">Water, Electricity, Healthcare, etc.</div>
                                </button>
                            </div>
                        </div>
                        ...
                        {officialType && (
                            <>
                                ...
                                {/* Conditional Fields Based on Official Type */}
                                {officialType === "MLA" ? (
                                    <>
                                        {/* Constituency (for MLAs) */}
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                    ...
                                    <>
                                        {/* District & Department (for Department Officials) */}
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition appearance-none bg-white"
                                                    required
                                                >
                                        ...
                                        {/* Designation (for Department Officials) */}
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                            ...
                                            <input
                                                ...
                                                placeholder="e.g., Assistant Engineer, Junior Officer"
                                                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition"
                                            />
                                        </div>
                                    </>
